...
object GraphxDemo {
  ...
  //  vertices2.collect().foreach(println(_))
    ...
    graphx.mapVertices((id,attr)=>attr._1+":"+attr._2)
      .triplets
      .collect
    ...
    println("subgraph: ")
    //满足三元组边判断，满足顶点的判定
    graphx.subgraph(x=>if(x.attr=="同事") true else false,(id,attr)=>true).vertices.collect().foreach(println(_))
    graphx.subgraph(x=>if(x.attr=="同事") true else false,(id,attr)=>true).edges.collect().foreach(println(_))
    ...
    graphx.collectNeighbors(EdgeDirection.Either).collect().foreach(f=>{
      ...
      println("VertexId:"+f._1)
      colls.foreach(print(_))
      println()
    })
    ...
    graphx.collectNeighborIds(EdgeDirection.Either).collect().foreach(println(_))
